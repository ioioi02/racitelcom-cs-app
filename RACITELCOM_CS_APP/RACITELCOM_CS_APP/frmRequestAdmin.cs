...
{
    public partial class frmRequestAdmin : Form
    {
        public frmRequestAdmin()
        {
            InitializeComponent();
        }

        private void frmRequestAdmin_Load(object sender, EventArgs e)
        {
            getRequestStatus(2);
            getRequestStatus(5);
            getRequestStatus(4);
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            int? account_update_status = reader.IsDBNull(reader.GetOrdinal("account_update_status")) ? (int?)null : reader.GetInt32(reader.GetOrdinal("account_update_status"));
        ...
        {
            removeAllControls();
            frmRequestAdmin_Load(null, null);
        }
    }
}
