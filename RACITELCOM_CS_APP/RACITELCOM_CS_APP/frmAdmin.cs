    ...
    public partial class frmAdmin : Form
    {
        ...
        private void Client_On_MessageReceived(string message)
        {
            if (InvokeRequired)
            {
                Invoke(new Action(() => {
                    ...
                    {
                        int count = countOpenTicket();
                        if (count > 0)
                        {
                            btnTicket.Text = $"TICKET ({count})";
                        }
                        else
                        {
        ...
        private int countOpenTicket()
        {
            ...
            try
            {
                ...
                string query = @"SELECT COUNT(*) AS openTicket FROM tbl_ticket WHERE ticket_type = 'Open';";
                using (MySqlCommand cmd = new MySqlCommand(query, connection))
                {
                    int openTicketCount = Convert.ToInt32(cmd.ExecuteScalar());
        ...
        private void btnDashboard_Click(object sender, EventArgs e)
        {
            ...
            if (btn_ != btn)
            {
                openFormContent(new frmDashboard(), null);
        ...
        private void btnCSPortal_Click(object sender, EventArgs e)
        {
            btn_ = sender as Button;
            if (btn_ != btn)
            {
        ...
        private void btnAccountingPortal_Click(object sender, EventArgs e)
        {
            btn_ = sender as Button;
            if (btn_ != btn)
            {
                openFormContent(new frmAccountingPortal(), null);
                btnHighlight(btn, btn_);
            }
            btn = btn_;
            lblHeaderText.Text = btn.Text;
        ...
        private void btnTechnicalPortal_Click(object sender, EventArgs e)
        {
            btn_ = sender as Button;
            if (btn_ != btn)
            {
                openFormContent(new frmTechnicalPortal(), null);
                btnHighlight(btn, btn_);
            }
            btn = btn_;
            lblHeaderText.Text = btn.Text;
        ...
        private void btnTicket_Click(object sender, EventArgs e)
        {
            ...
            if (btn_ != btn)
            {
                openFormContent(new frmPendingTicket(), null);
                btnHighlight(btn, btn_);
            }
            btn = btn_;
            lblHeaderText.Text = btn.Text;
        ...
        private void btnRequest_Click(object sender, EventArgs e)
        {
            ...
            if (btn_ != btn)
            {
                openFormContent(new frmRequestAdmin(), null);
                btnHighlight(btn, btn_);
            }
            btn = btn_;
            lblHeaderText.Text = btn.Text;
        ...
        private void btnManageUser_Click(object sender, EventArgs e)
        {
            ...
            if (btn_ != btn)
            {
                frmManageUser manageUser = new frmManageUser(this);
                manageUser.main_user_name = username;
                openFormContent(manageUser, null);
                btnHighlight(btn, btn_);
            }
            btn = btn_;
            lblHeaderText.Text = btn.Text;
